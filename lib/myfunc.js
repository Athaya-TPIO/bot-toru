...
exports.saveCustomCommands = (commands) => {
  fs.writeFileSync(jsonFilePath, JSON.stringify(commands, null, 2), 'utf8');
}

exports.handleCustomCommands = (groupID, command, reply) => {
...
exports.addCustomCommand = (groupID, command, response) => {
  const customCommands = readCustomCommands();
  if (!customCommands[groupID]) {
...
exports.connect = async (endPoint, postData, contentType = "application/json") => {
    let options;
    let payload;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { response: false, data: { msg: `HTTP Error: ${response.status}` } };
        }

        const data = await response.json(); // Menguraikan response JSON
    
        return data; // Mengembalikan data ke pemanggil
    } catch (error) {
        console.error("Error in API call:", error);
        return { response: false, data: { msg: "Request Error" } };
    }
};

exports.generateUniqueRefID = () => {
